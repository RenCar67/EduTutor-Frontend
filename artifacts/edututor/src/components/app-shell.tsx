import { Bell, BookOpen, ChevronDown, CircleHelp, Database, LayoutDashboard, LineChart, ListChecks, Menu, Search, ShieldCheck, SlidersHorizontal, UsersRound, X } from 'lucide-react';
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  }), [mockMode, role, services, sessions, audit]);
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[252px] flex-col border-r border-sidebar-border bg-sidebar px-4 py-5 text-sidebar-foreground transition-transform duration-300 md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-auto space-y-3">
          <div className="rounded-2xl border border-sidebar-border bg-sidebar-accent/50 p-3">
            ...
            <p className="mt-2 text-[10px] leading-relaxed text-sidebar-foreground/40">{mockMode ? 'Demo workspace · local data' : 'Live API · synced workspace'}</p>
          </div>
          <div className="flex items-center gap-3 border-t border-sidebar-border px-2 pt-4"><span className="grid h-8 w-8 place-items-center rounded-full bg-accent font-display text-sm font-bold text-accent-foreground">MG</span><div className="min-w-0"><p className="truncate text-[12px] font-semibold">Mariana García</p><p className="truncate text-[10px] text-sidebar-foreground/45">Operations lead</p></div><CircleHelp size={15} className="ml-auto text-sidebar-foreground/35" /></div>
        </div>
      </aside>
